...
describe('Pi workflow engine dependencies', () => {
  it('provides operational engine and platform dependency callbacks', async () => {
    ...
    await expect(captured.engineDeps.sessionContext.isSubagent()).resolves.toBe(false)
    await expect(captured.engineDeps.sessionContext.getMainSessionId()).resolves.toBe('session-id')
