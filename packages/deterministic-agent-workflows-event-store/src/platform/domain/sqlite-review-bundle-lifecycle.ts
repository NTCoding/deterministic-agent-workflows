import { z } from 'zod'
import type { StoredReviewBundle } from '@nt-ai-lab/deterministic-agent-workflow-engine'
...
export function updateReviewBundleLifecycle(
  ...
  db.prepare(
    'UPDATE review_bundles SET status = ?, updated_at = ?, failure_reason = ? WHERE bundle_id = ?',
  ).run(status, updatedAt, reason ?? null, current.bundleId)
