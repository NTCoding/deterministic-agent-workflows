...
type StatePeriod = { readonly state: string; readonly startedAt: string; readonly endedAt?: string | undefined }
type JournalEntry = { readonly agentName: string; readonly content: string; readonly at: string; readonly state: string }
type InsightEntry = { readonly severity: string; readonly title: string; readonly evidence: string }
...
type PriceRow = { readonly input: number; readonly output: number; readonly cacheRead: number; readonly cacheWrite: number }
...
function priceFor(model: string | undefined): PriceRow {
  if (!model) return { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }
...
function miniPreview(content: ReadonlyArray<TranscriptContentBlock>): string {
  for (const b of content) {
    if (b.kind === 'text') return b.text.replace(/\s+/g, ' ').slice(0, 80)
    if (b.kind === 'tool_use') return `⚙ ${b.name}`
    if (b.kind === 'thinking') return `🧠 ${b.text.replace(/\s+/g, ' ').slice(0, 70)}`
    if (b.kind === 'tool_result') return `↩ ${b.toolName}`
  }
  return ''
}
...
function renderToolUseBlock(id: string, name: string, input: Record<string, unknown>): string {
  ...
  const val = key ? String(Object.values(input)[0] ?? '').slice(0, 80).replace(/\n/g, ' ') : ''
...
export function attachTranscriptListeners(): void {
  ...
  document.querySelectorAll<HTMLAnchorElement>('.tr-mm-item, .tr-mm-transition').forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      e.preventDefault()
      const row = document.getElementById(`msg-${idx}`)
      if (row) { row.scrollIntoView({ behavior: 'smooth', block: 'start' }); flashMessage(`msg-${idx}`) }
  ...
  if (m) {
    ...
    setTimeout(() => {
      ...
      if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'start' }); flashMessage(id) }
    }, 50)
  }
}
