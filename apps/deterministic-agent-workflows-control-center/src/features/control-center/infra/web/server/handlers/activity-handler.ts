...
function buildActivityReport(calls: ReadonlyArray<ToolCall>): ActivityReport {
  ...
  return {
    ...
    bashCommands: topN(bashCounts, 15).map(([command, count]) => ({
      command,
      count 
    })),
    ...
    grepSearches: topN(grepCounts, 12).map(([pattern, count]) => ({
      pattern,
      count 
    })),
    globSearches: topN(globCounts, 12).map(([pattern, count]) => ({
      pattern,
      count 
    })),
    tasksDelegated: tasks.slice(0, 30),
    webFetches: topN(webFetch, 15).map(([url, count]) => ({
      url,
      count 
    })),
    webSearches: topN(webSearch, 15).map(([url, count]) => ({
      url,
      count 
    })),
  }
...
function loadStatePeriods(deps: SessionQueryDeps, sessionId: string): Array<{
  state: string;
  startedAt: string;
...
export function handleGetSessionActivity(
  ...
  return (_req, res, route) => {
    ...
    const periods: StatePeriod[] = periodsRaw.map(p => ({
      state: p.state,
    ...
    if (unassignedCalls.length > 0) {
      const initial = getInitialState(deps.queryDeps, sessionId)
      ...
      byState.push({
        ...
        startedAt: initial?.startedAt ?? (firstTs > 0 ? new Date(firstTs).toISOString() : ''),
