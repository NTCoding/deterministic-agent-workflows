...
export async function renderSessionCompare(container: HTMLElement, idA: string, idB: string): Promise<void> {
  ...
  try {
    ...
    const stateDistA = sessionA.statePeriods.map((p) => ({
      label: p.state,
      value: p.durationMs,
      color: stateColor(p.state) 
    }))
    const stateDistB = sessionB.statePeriods.map((p) => ({
      label: p.state,
      value: p.durationMs,
      color: stateColor(p.state) 
    }))
